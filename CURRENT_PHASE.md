...
- **Post-launch: "Option A" full-bleed REVERSED to "Option B" framed side thumbnail — 2026-07-21, reviewed + deployed, confirmed live.** The PO reviewed the Option A full-bleed autoplay preview (`9a0aa42`) live and asked for it to be replaced: full-bleed media filled each row and fought the text. Option B restores a framed thumbnail on the RIGHT (20rem box at `lg`, `aspect-[16/10]`, stacking full-width below `lg`) with title/text on the LEFT on plain `#0A0A0A` background — text and media never overlap, so contrast is trivial (no scrim needed). The **in-view (scroll-into-view) autoplay mechanism is kept unchanged** — the same `use-in-view-autoplay.ts` IntersectionObserver hook now slides/fades the whole thumbnail box in on the `active` signal; pause-out-of-view, reduced-motion static-poster, Tiger Kick image-only, and JPD typographic-only all preserved. Single shared `components/projects/project-media-row.tsx` still backs both `/projects` work rows and the homepage Featured section. Full mandatory chain: architect (2026-07-22 ruling, explicitly supersedes but keeps the 2026-07-21 Option A entry in docs/DESIGN_SYSTEM.md for history) -> frontend-engineer -> code-reviewer PASS -> qa-engineer PASS (independent Lighthouse run matched, third consistent measurement). Final independently-measured Lighthouse: **`/` Perf 94 / A11y 100 / BP 100 / SEO 100 / CLS 0; `/projects` Perf 96 / A11y 100 / BP 100 / SEO 100 / CLS 0** — both clear the amended gate; /projects Perf improved vs Option A. Commit `4c748d7`; producer confirmed live over HTTPS at jod.aiklaotrip.com (both routes 200 serving the Option B two-column grid — `grid-template-columns:minmax(0,1fr) 20rem` + `aspect-ratio:16/10` present in served HTML/compiled CSS, zero Option A `bg-gradient-to-r` scrim; fresh CSS + JS chunks 200). No backlog card was filed — the change went straight through the review chain.
