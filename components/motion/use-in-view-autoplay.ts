...
/**
 * useInViewAutoplay — Option A in-view autoplay (architect ruling 2026-07-21,
 * DESIGN_SYSTEM.md decision log). Drives the full-bleed project preview media
 * on BOTH `/projects` and the home `01 — Featured projects` section.
 *
 ...
 * - `active` (returned) = inView && !reducedMotion. Drives the VISUAL
 *   title-only + scrim state (via a `data-active` attr on the row). Hover/focus
 *   contribute to the visual state via CSS (`group-hover`/`group-focus-within`)
 *   in the component — they do NOT start/stop playback.
 * - SSR-safe: IntersectionObserver + the reactive media query are only touched
 *   in effects; the server render is `active = false` (full descriptive text).
